...
(
	...
	CONSTRAINT FK_USUARIO_PROG FOREIGN KEY (oid_recursos)
        ...
        ON UPDATE NO ACTION
        ON DELETE NO ACTION ,
    CONSTRAINT FK_USUARIO_PROG FOREIGN KEY (nm_recursos)
        REFERENCES db_projetos_ib.recursos (nm_recursos) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);
...
(
   ...
   nm_empresa character varying(100) COLLATE pg_catalog."default" NOT NULL,
    ...
    CONSTRAINT fk_prog_projetos_prog FOREIGN KEY (vr_custo_aprovado)
        REFERENCES db_projetos_ib.centro_custo (  vr_custo_aprovado) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_projetos_prog FOREIGN KEY ( nm_empresa)
        REFERENCES db_projetos_ib.empresa ( nm_empresa) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);
...
(
    ...
    CONSTRAINT fk_prog_atividades_prog FOREIGN KEY (nm_projetos)
        REFERENCES db_projetos_ib.projetos (nm_projetos) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT fk_atividades_prog FOREIGN KEY (nm_recursos)
        REFERENCES db_projetos_ib.recursos (nm_recursos) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);
...
(
    ...
    CONSTRAINT fk_apontamentos_prog FOREIGN KEY (nm_recursos)
        REFERENCES db_projetos_ib.atividades (nm_recursos) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);